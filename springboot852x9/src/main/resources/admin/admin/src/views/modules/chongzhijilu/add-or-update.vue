	...
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px","background":"url(http://codegen.caihongy.cn/20230401/bd196cf1072f48afae5676066a72e297.jpg) fixed no-repeat center top / 100% 100%"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="充值金额" prop="qianbaojine">
					<el-input v-model="ruleForm.qianbaojine" placeholder="充值金额" clearable  :readonly="ro.qianbaojine"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="充值金额" prop="qianbaojine">
					<el-input v-model="ruleForm.qianbaojine" placeholder="充值金额" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="充值时间" prop="chongzhishijian">
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.chongzhishijian" 
						type="datetime"
						:readonly="ro.chongzhishijian"
						placeholder="充值时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.chongzhishijian" label="充值时间" prop="chongzhishijian">
					<el-input v-model="ruleForm.chongzhishijian" placeholder="充值时间" readonly></el-input>
				</el-form-item>
			</template>
...
export default {
	...
	created() {
		this.ruleForm.chongzhishijian = this.getCurDateTime()
	},
	methods: {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `chongzhijilu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.chongzhijiluCrossAddOrUpdateFlag = false;
						 this.parent.search();
