	...
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px","background":"url(http://codegen.caihongy.cn/20230401/bd196cf1072f48afae5676066a72e297.jpg) fixed no-repeat center top / 100% 100%"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="提醒时间" prop="tixingshijian">
					<el-date-picker
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.tixingshijian" label="提醒时间" prop="tixingshijian">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="提醒内容" prop="tixingneirong">
					<editor 
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.tixingneirong" label="提醒内容" prop="tixingneirong">
                    <span :style='{"fontSize":"16px","lineHeight":"40px","color":"#666","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.tixingneirong"></span>
                </el-form-item>
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.tixingneirong = this.ruleForm.tixingneirong.replace(reg,'../../../springboot852x9/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `yonghutixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.yonghutixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
