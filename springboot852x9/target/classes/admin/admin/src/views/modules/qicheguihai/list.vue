<template>
	<div class="main-content" :style='{"minHeight":"100vh","padding":"30px","background":"url(http://codegen.caihongy.cn/20230401/bd196cf1072f48afae5676066a72e297.jpg) fixed no-repeat center top / 100% 100%"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 0px"}' :inline="true" :model="searchForm">
				...
				<el-row :style='{"padding":"16px 12px 10px","margin":"0 0 0px","borderColor":"#e2f0e2","flexWrap":"wrap","background":"none","borderWidth":"0px","display":"flex","width":"auto","borderStyle":"solid","zIndex":"999"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 16px","margin":"0 10px 6px 0","borderColor":"#ccdeee","color":"#333","outline":"none","borderRadius":"4px","background":"linear-gradient(90deg, rgba(240,248,255,1) 0%, rgba(255,255,255,1) 50%, rgba(240,248,255,1) 100%)","borderWidth":"1px","width":"auto","fontSize":"16px","borderStyle":"solid","height":"36px"}' v-if="isAuth('qicheguihai','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 16px","margin":"0 10px 6px 0","borderColor":"#ccdeee","color":"#333","outline":"none","borderRadius":"4px","background":"linear-gradient(90deg, rgba(240,248,255,1) 0%, rgba(255,255,255,1) 50%, rgba(240,248,255,1) 100%)","borderWidth":"1px","width":"auto","fontSize":"16px","borderStyle":"solid","height":"36px"}' v-if="isAuth('qicheguihai','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>


					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 16px","margin":"0 10px 6px 0","borderColor":"#ccdeee","color":"#333","outline":"none","borderRadius":"4px","background":"linear-gradient(90deg, rgba(240,248,255,1) 0%, rgba(255,255,255,1) 50%, rgba(240,248,255,1) 100%)","borderWidth":"1px","width":"auto","fontSize":"16px","borderStyle":"solid","height":"36px"}' v-if="isAuth('qicheguihai','审核')" :disabled="dataListSelections.length <= 0" type="danger" @click="shBatchDialog()">批量审核</el-button>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' v-if="isAuth('qicheguihai','审核')" prop="sfsh" label="审核">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"border":"0px solid #1e98f5","cursor":"pointer","padding":"0 10px 0 24px","margin":"0 5px 6px","outline":"none","color":"#1e98f5","borderRadius":"0px","background":"url(http://codegen.caihongy.cn/20230329/38553f099fa74a2d86e2206c373a9539.png) no-repeat left center / 20px","width":"auto","fontSize":"15px","height":"32px"}' v-if=" isAuth('qicheguihai','查看')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button :style='{"border":"0px solid #1e98f5","cursor":"pointer","padding":"0 10px 0 24px","margin":"0 5px 6px","outline":"none","color":"#1e98f5","borderRadius":"0px","background":"url(http://codegen.caihongy.cn/20230329/38553f099fa74a2d86e2206c373a9539.png) no-repeat left center / 20px","width":"auto","fontSize":"15px","height":"32px"}' v-if="isAuth('qicheguihai','租用费用')" type="success" size="mini" @click="zuyongfeiyongCrossAddOrUpdateHandler(scope.row,'cross','是','[1]','已提交计费')">租用费用</el-button>
							<el-button :style='{"border":"0px solid #06d5dd","cursor":"pointer","padding":"0 10px 0 24px","margin":"0 5px 6px","outline":"none","color":"#15a4aa","borderRadius":"0px","background":"url(http://codegen.caihongy.cn/20230329/99f867042acb4c4aae637c8bfcf6714a.png) no-repeat left center","width":"auto","fontSize":"15px","height":"32px"}' v-if=" isAuth('qicheguihai','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>





							<el-button :style='{"border":"0px solid #c00","cursor":"pointer","padding":"0 10px 0 24px","margin":"0 5px 6px","outline":"none","color":"#c00","borderRadius":"0px","background":"url(http://codegen.caihongy.cn/20230329/c5ea125f385740a3a3b9aed07f5dfe2c.png) no-repeat left 6px / 20px","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('qicheguihai','删除') " type="danger" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
		...
		<zuyongfeiyong-cross-add-or-update v-if="zuyongfeiyongCrossAddOrUpdateFlag" :parent="this" ref="zuyongfeiyongCrossaddOrUpdate"></zuyongfeiyong-cross-add-or-update>
...
import zuyongfeiyongCrossAddOrUpdate from "../zuyongfeiyong/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      zuyongfeiyongCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    zuyongfeiyongCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
	if(crossOptAudit=='是'&&row.sfsh!='是') {
      ...
      this.zuyongfeiyongCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','qicheguihai');
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.zuyongfeiyongCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.zuyongfeiyongCrossaddOrUpdate.init(row.id,type);
      });
    },
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
          chuzudanhao: row.chuzudanhao,
          qichebianhao: row.qichebianhao,
          qichemingcheng: row.qichemingcheng,
          qicheleixing: row.qicheleixing,
          qichepaihao: row.qichepaihao,
          zuhuzhanghao: row.zuhuzhanghao,
          shoujihaoma: row.shoujihaoma,
          rizujiage: row.rizujiage,
          zulinshijian: row.zulinshijian,
    ...
    shBatchHandler(){
      this.$confirm(`确定一键审核操作?`, "提示", {
        ...
        this.$http({
          url: "qicheguihai/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
          method: "post",
          data: this.batchIds
        }).then(({ data }) => {
          if (data && data.code === 0) {
